...
| `WorkerResultEvidenceGate` | normalizes command evidence for | `CrossPlatformCI` | `insights/live-smoke-lessons.md`, todo-list-test-17 run `run-build-local-todo-app-20260528-0926`, and `tests/test_worker_backends.py` | confirmed | 2026-05-28 | Normalize shell wrappers and path separators before declaring Worker Result `tests_run` evidence missing. |
| `PlanningSQLite` | rejects hidden failed controller evidence for | `StoryLoop` | `insights/live-smoke-lessons.md`, todo-list-test-17 `controller.stdout.json`, `scripts/check_planning_integrity.py`, and `tests/test_planning_integrity.py` | confirmed | 2026-05-28 | Do not mark a failed controller run completed in place; create explicit recovery evidence. |
