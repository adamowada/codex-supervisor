...
During first bootstrap, run dependency setup first or allow `uv run` to sync; reserve `--no-sync`
for already-synced write environments and for verification evidence.

```sh
uv run python -B -m codex_supervisor.cli plan-init --seed-bootstrap-plan
uv run python -B -m codex_supervisor.cli plan-upsert ...
uv run python -B -m codex_supervisor.cli milestone-upsert ...
uv run python -B -m codex_supervisor.cli criterion-upsert ...
uv run python -B -m codex_supervisor.cli decision-add ...
uv run python -B -m codex_supervisor.cli progress-add ...
uv run python -B -m codex_supervisor.cli artifact-link-add ...
uv run python -B -m codex_supervisor.cli commit-link-add ...
uv run python -B -m codex_supervisor.cli task-upsert ...
uv run python -B -m codex_supervisor.cli task-claim --task-id <task-id> --worker-run-id <worker-run-id> ...
uv run python -B -m codex_supervisor.cli worker-run-upsert ...
uv run python -B -m codex_supervisor.cli plan-status ...
