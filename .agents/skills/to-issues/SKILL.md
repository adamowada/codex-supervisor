...
If the current user turn is read-only, review-only, audit-only, or no-edits mode, do not create
issues, mutate planning SQLite, or edit files. Return issue drafts and the exact proposed durable
updates instead.
