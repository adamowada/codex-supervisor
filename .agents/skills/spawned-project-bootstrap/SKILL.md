...
4. Seed planning SQLite with a bootstrap plan and one next actionable task. Use HITL honestly for
   unresolved product, credential, privacy, or publication decisions.
...
7. Record any copied, adapted, or inspiration source material in attribution files before publication.
8. Run focused checks first, then the broad gate. If publication is blocked by untracked files,
