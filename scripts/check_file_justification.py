...
FILE_PURPOSES = {
    ...
    "src/codex_supervisor/mcp_server.py": FilePurpose(
    ...
    "src/codex_supervisor/runtime_preflight.py": FilePurpose(
        "supervisor runtime preflight ledger and fail-closed mode checks", "pytest"
    ),
    ...
    "tests/test_runtime_preflight.py": FilePurpose(
        "supervisor runtime preflight ledger tests", "pytest"
    ),
...
def check_file_justification(repo_root: Path) -> tuple[FileJustificationFailure, ...]:
    ...
    for relative_path in relative_paths:
        if _insights_non_markdown_reason(relative_path) is not None:
            failures.append(
                FileJustificationFailure(
                    relative_path,
                    "insights/ is reserved for synthesized durable learning markdown",
                )
            )
...
def _insights_non_markdown_reason(relative_path: str) -> str | None:
    normalized = relative_path.replace("\\", "/")
    if normalized.startswith("insights/") and Path(normalized).suffix.lower() != ".md":
        return "non_markdown"
    return None
