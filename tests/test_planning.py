...
def test_cli_write_commands_record_planning_rows(tmp_path, capsys):
    ...
    _write_worker_result(
        tmp_path,
        result_path="insights/run-write-result.json",
        worker_run_id="run-write",
        changed_files=("src/write.py",),
        verification_commands=("python -B -m pytest -p no:cacheprovider",),
    ...
    assert (
        main(
            [
                ...
                "completed",
                "--result-path",
                "insights/run-write-result.json",
                "--metadata-json",
                '{"ok":true}',
            ]
        )
    ...
    assert worker_run["result_id"].startswith("worker-result-insights-run-write-result.json-")
...
def test_cli_status_commands_update_lifecycle_state(tmp_path, capsys):
    ...
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "status.py").write_text("print('ok')\n", encoding="utf-8")
    _write_worker_result(
        tmp_path,
        result_path="insights/run-status-result.json",
        worker_run_id="run-status",
        changed_files=("src/status.py",),
        verification_commands=("python -B -m pytest -p no:cacheprovider",),
        acceptance_criteria=("Criterion",),
    )
    store.upsert_worker_run(
    ...
    commands = [
        ...
        [
            ...
            "completed",
            "--result-path",
            "insights/run-status-result.json",
            "--completed-at",
...
def test_worker_run_status_rejects_blank_result_path_without_erasing_existing_result_id(tmp_path):
    ...
    assert run.result_id.startswith("worker-result-runs-run-test-result.json-")
...
def test_worker_run_upsert_completed_validates_worker_result_contract(tmp_path, capsys):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Exercise worker run upsert validation.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
    ...
    assert (
        main(
            [
                ...
                "completed",
                "--result-path",
                "insights/missing-result.json",
            ]
        )
        == 1
    )
    assert "worker result does not exist" in capsys.readouterr().err
    run = open_existing_planning_database(db_path).list_worker_runs(task_id="task-test")[0]
    assert run.status == "failed"
    assert run.failure_class == "worker_result_invalid"
    assert run.result_path == "insights/missing-result.json"
...
def test_worker_run_status_clears_failure_class_on_completion(tmp_path):
    db_path = tmp_path / "plans" / "planning.sqlite3"
    store = initialize_planning_database(db_path)
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Exercise worker completion evidence cleanup.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
    ...
    store.upsert_worker_run(
        WorkerRunRecord(
            ...
            backend="codex_exec",
            status="failed",
            result_path="insights/run-test-failed.json",
...
def test_worker_result_ingestion_completes_after_validation(tmp_path):
    db_path = tmp_path / "plans" / "planning.sqlite3"
    store = initialize_planning_database(db_path)
    store.upsert_plan(
        PlanRecord(
...
def test_worker_result_ids_include_source_path_for_repeated_filenames(tmp_path):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Avoid worker result ID collisions.",
            status="active",
        )
    )
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "worker.py").write_text("print('ok')\n", encoding="utf-8")
    for worker_run_id in ("run-a", "run-b"):
        store.upsert_supervisor_task(
            SupervisorTaskRecord(
                task_id=f"task-{worker_run_id}",
                plan_id="plan-test",
                title=f"Task {worker_run_id}",
                goal="Complete through result ingestion.",
                task_type="AFK",
                status="ready",
                acceptance_criteria=["Criterion passes."],
    ...
    result_a = store.ingest_worker_result("run-a", "artifacts/run-a/worker-result.raw.json")
    result_b = store.ingest_worker_result("run-b", "artifacts/run-b/worker-result.raw.json")

    assert result_a.result_id != result_b.result_id
    assert result_a.result_id.startswith("worker-result-artifacts-run-a-worker-result.raw.json-")
...
def test_worker_result_reingestion_replaces_stale_run_links(tmp_path):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Replace stale result links.",
            status="active",
        )
    )
    (tmp_path / "src").mkdir()
    ...
    old_result = store.ingest_worker_result("run-test", "runs/run-test/result.json")
    _write_worker_result(
        tmp_path,
    ...
    new_result = store.ingest_worker_result("run-test", "artifacts/run-test/worker-result.raw.json")

    read_store = open_existing_planning_database(db_path)
    run = read_store.list_worker_runs(task_id="task-test")[0]
    links = read_store.list_worker_result_run_links()
    result_ids = {result.result_id for result in read_store.list_worker_results()}
    assert old_result.result_id not in result_ids
    assert run.result_id == new_result.result_id
    assert [(link.result_id, link.worker_run_id) for link in links] == [
        (new_result.result_id, "run-test")
    ]
...
def test_worker_result_ingestion_failure_preserves_evidence_without_completion(tmp_path):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Reject invalid worker results.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
        SupervisorTaskRecord(
            ...
            plan_id="plan-test",
            title="Task",
            goal="Fail through invalid result ingestion.",
            task_type="AFK",
            status="ready",
            acceptance_criteria=["Criterion passes."],
            verification_commands=["python -B -m pytest -p no:cacheprovider"],
    ...
    claim = store.claim_next_ready_afk_task(worker_run_id="run-test", backend="codex_exec")
    assert claim is not None
    result_path = tmp_path / "runs" / "run-test" / "result.json"
        ...
        json.dumps(
            {
                ...
                "risks": ["bad"],
                "follow_up_tasks": [],
                "artifacts": ["runs/run-test/result.json"],
                "handoff_notes": "Nope.",
            }
        ),
        encoding="utf-8",
    ...
    with pytest.raises(ValueError, match="tests_run"):
        store.ingest_worker_result("run-test", "runs/run-test/result.json")
...
def test_worker_result_ingestion_preserves_noncompleted_worker_result_status(
    tmp_path,
    status,
):
    db_path = tmp_path / "plans" / "planning.sqlite3"
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Preserve worker result status.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
        SupervisorTaskRecord(
            ...
            plan_id="plan-test",
            title="Task",
            goal="Ingest noncompleted result.",
            task_type="AFK",
            status="ready",
            acceptance_criteria=["Criterion passes."],
            verification_commands=["python -B -m pytest -p no:cacheprovider"],
    ...
    result = store.ingest_worker_result("run-test", "runs/run-test/result.json")
...
def test_worker_result_ingestion_needs_review_preserves_evidence(tmp_path):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Preserve needs_review worker result status.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
        SupervisorTaskRecord(
            ...
            plan_id="plan-test",
            title="Task",
            goal="Ingest needs_review result.",
            task_type="AFK",
            status="ready",
            acceptance_criteria=["Criterion passes."],
            verification_commands=["python -B -m pytest -p no:cacheprovider"],
...
def test_worker_result_ingestion_rejects_uncompleted_shared_worker_run(tmp_path):
    ...
    store.upsert_plan(
        PlanRecord(
            ...
            slug="test",
            title="Test Plan",
            goal="Validate shared worker result membership.",
            status="active",
        )
    )
    store.upsert_supervisor_task(
...
def _write_worker_result(
    ...
    *,
    ...
    payload: dict[str, object] = {
        ...
        "acceptance_results": {
            criterion: {"status": "passed", "evidence": "Evidence."}
            for criterion in acceptance_criteria
        },
        "risks": [],
        "follow_up_tasks": [],
        "artifacts": list(artifacts) if artifacts is not None else [result_path],
        "handoff_notes": "Ready.",
    }
    if worker_run_ids is None:
