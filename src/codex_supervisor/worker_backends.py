...
from codex_supervisor.worker_evidence import (
    WorkerRunEventSink,
    jsonl_validation_failure,
    summarize_codex_exec_stream_event,
    worker_result_evidence_validation_failure,
)
...
class WorkerLaunchRequest:
    ...
    event_sink: WorkerRunEventSink | None = field(default=None, repr=False, compare=False)
...
class _CodexExecStreamObserver:
    ...
    def __init__(
        ...
        *,
        ...
        self._event_sink = request.event_sink
    ...
    def _record_stdout_line(self, line: bytes) -> None:
        ...
        if not isinstance(payload, dict):
            return
        event = summarize_codex_exec_stream_event(payload)
        if event is None:
            return
        self._event_index += 1
        ...
        if event_sink is None:
            return
        _safe_observer_call(
            lambda: event_sink.record_stream_event(
...
def _jsonl_validation_failure(request: WorkerLaunchRequest) -> JsonObject | None:
    return jsonl_validation_failure(
        repo_root=request.repo_root,
        jsonl_path=request.jsonl_path,
        allow_degraded_jsonl=request.allow_degraded_jsonl,
    )
...
def _worker_result_evidence_validation_failure(
    ...
    return worker_result_evidence_validation_failure(
        repo_root=request.repo_root,
        jsonl_path=request.jsonl_path,
