...
- Prefer AFK-ready task contracts, but mark HITL honestly when judgment, secrets, external access, design approval, or production action is required.
...
4. **Contract.** Ensure the current task has goal, scope, out-of-scope, acceptance criteria, verification commands, allowed paths, blockers, and HITL/AFK classification.
...
6. **Execute.** Implement one vertical slice or repair loop at a time. Keep unrelated refactors out unless the selected skill says they are part of the task.
