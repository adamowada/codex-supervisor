...
def test_attempt_store_runs_lifecycle_and_attaches_evidence(tmp_path: Path) -> None:
    db_path = _make_planning_db(tmp_path)
...
def test_attempt_store_rejects_evidence_for_another_task(tmp_path: Path) -> None:
    ...
    _insert_task(db_path, task_id="task-2", status="ready")
...
def _make_planning_db(tmp_path: Path) -> Path:
    ...
    try:
        ...
        connection.execute("insert into meta(key, value) values ('schema_name', 'test')")
        ...
        connection.close()
    _insert_task(db_path, task_id="task-1", status="ready")
    return db_path
    ...
    try:
        connection.execute(
            """insert into tasks(
               ...
               ) values (?, 'plan-1', ?, ?, 'medium', 'Implement work', ?, ?, ?)""",
            (
                task_id,
                task_id,
                status,
